...
router.post('/start', function(req, res, next) {
    ...
    const args = [
        ...
        '-threads', '0',
        '-sn',
        '-pix_fmt', `${outputPixFormat}`,
        '-c:v', 'h264_omx',
        '-b:v', `${outputVideoBitRate}k`,
        '-maxrate', `${outputVideoBitRate * 2}k`,
        '-bufsize', `${outputVideoBitRate * 4}k`,
        '-vsync', 'vfr',
        '-level', '41',
        '-sc_threshold', '0',
        '-b:a', `${outputAudioBitRate}k`,
        '-c:a', 'aac',
        '-g', `${inputFrameRate * 2}`,
        '-f', 'mpegts',
        '-flush_packets', '0', `${serverUrl}`
    ];
